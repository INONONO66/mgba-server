...
- `--instances N` creates or validates up to the supported 20-instance target. Values above 20 are rejected. Strict acceptance only passes with exactly `20`.
...
See `docs/strict-sustained-validation.md` for the captured strict report format and exact validation assumptions.
...
- `CAPTURE_ROOT` controls the host root for per-instance capture directories. Default: `/tmp/grokemon-captures`.
...
- `EMULATOR_MEMORY_BYTES` sets the Docker memory and swap cap per emulator. Default: `805306368` bytes, keeping twenty emulators within the 16 GiB strict RAM target before gateway/process overhead.
...
The benchmark does not bundle ROM assets, does not modify gameplay/LLM policy, does not fork mGBA, and targets up to 20 concurrent instances.

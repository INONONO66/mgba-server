...
describe('DockerDriver', () => {
  ...
  it('creates emulator containers with host-mounted capture storage and bounded runtime resources', async () => {
    ...
    expect(mkdir).toHaveBeenCalledWith('/tmp/grokemon-captures-test', { recursive: true, mode: 0o700 })
    expect(realpath).toHaveBeenCalledWith('/tmp/grokemon-captures-test')
    expect(mkdir).toHaveBeenCalledWith('/tmp/grokemon-captures-test/instance-a', { recursive: true, mode: 0o700 })
    ...
    expect(dockerMock.createOptions[0]).toMatchObject({
      ...
      HostConfig: {
        Mounts: [
          { Target: '/capture', Source: '/tmp/grokemon-captures-test/instance-a', Type: 'bind' },
  ...
  it('cleans up the container and capture directory when port inspection fails', async () => {
    ...
    expect(rm).toHaveBeenCalledWith('/tmp/grokemon-captures-test/instance-a', { force: true, recursive: true })
  })
  ...
  it('tightens permissive capture directory permissions before mounting', async () => {
    ...
    expect(chmod).toHaveBeenCalledWith('/tmp/grokemon-captures-test', 0o700)
    expect(chmod).toHaveBeenCalledWith('/tmp/grokemon-captures-test/instance-a', 0o700)
  })

...
```text
${CAPTURE_ROOT:-/tmp/grokemon-captures}/<instance-id>:/capture:rw
```
...
`InstanceManager.destroy()` removes the per-instance capture directory after stopping the container. Managed containers carry a `pss-mgba.capture-directory` label so reconstruction can restore the host capture path when the gateway starts with existing managed containers. Reconstruction stops unlabeled or unsafe managed containers instead of adopting them. Cleanup rejects capture directories outside the configured `CAPTURE_ROOT`; if a stale label points outside the root, the container is stopped but that outside path is left for manual operator cleanup. Screenshot reads reject symlinks, non-regular files, and oversized capture files before reading bytes.

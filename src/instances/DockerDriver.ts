...
export class DockerDriver {
  ...
  async createContainer(opts: ContainerCreateOptions): Promise<ContainerInfo> {
    const containerName = `grokemon-${opts.instanceId}`
    ...
    try {
      ...
      if ((hostPort === undefined || hostPort === '') && (containerIp === undefined || containerIp === '')) {
        throw new Error('Container port not bound')
      }

      // Prefer host port binding — gateway runs on host, not inside Docker network
      const host = hostPort && hostPort !== '' ? '127.0.0.1' : (containerIp ?? '127.0.0.1')
      const port = hostPort && hostPort !== '' ? Number.parseInt(hostPort, 10) : opts.emulatorPort
